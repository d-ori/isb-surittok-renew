...
$(function () {
    $(".menu").load("/youhu/menu.html");
    $(".top").load("/youhu/top.html");
...
var Common = {
    ...
    axiosInstance: (function () {
        ...
        instance.interceptors.request.use(
            function (config) {
                const token = localStorage.getItem("jwtToken_youhu");
                if (token) {
        ...
        instance.interceptors.response.use(
            ...
            async (error) => {
                ...
                if (
                    ...
                    try {
                        const refreshToken_youhu =
                            localStorage.getItem("refreshToken_youhu");

                        if (!refreshToken_youhu) {
                            localStorage.clear();
                            // location.href = "/youhu/login.html";
                            return Promise.reject(error);
                        }
                        ...
                        const refreshRes = await axios.post(
                            ...
                            { login_token: refreshToken_youhu },
                            {
                                headers: {
                        ...
                        // 새로운 accessToken 저장
                        localStorage.setItem("jwtToken_youhu", newAccessToken);
    ...
    // paging
    renderPaging: function (searchVO) {
        const $container = $("#pagingArea");
        $container.empty();
        ...
        $container.append(
            $("<p>")
                .text(">")
                .click(() => {
                    if (searchVO.page + 1 <= searchVO.endedPageIndex) {
                        pageMove(searchVO.page + 1);
                    } else {
                        alert("마지막 페이지입니다.");
                    }
        ...
        $container.append(
            $("<p>")
                ...
                .click(() => {
                    if (searchVO.page + 1 <= searchVO.endedPageIndex) {
                        pageMove(searchVO.endedPageIndex);
                    } else {
                        alert("마지막 페이지입니다.");
                    }
                }),
    ...
    renderPaging_: function (searchVO, container = "#pagingArea") {
        ...
        $container.append(
            $("<p>")
                ...
                .click(() => {
                    if (searchVO.page + 1 <= searchVO.endedPageIndex) {
                        pageMove_(searchVO.endedPageIndex);
                    } else {
                        alert("마지막 페이지입니다.");
                    }
                }),
